...
Primary API: [PokeAPI](https://pokeapi.co/docs/v2#info) (specifically, their Generations, Pokemon Species, and Pokemon endpoints)
- Required to access: Nothing. This API is open to access for everybody, so no sign up or special API keys are required.
Supplementary Data Sources: No static data sources were used in the creation of this project, but outside information was accessed through other sources:
...
In terms of Python libraries, you will need the json, requests, pandas, plotly.express, and plotly.graph_objects libraries. These libraries, as well as import statements, are provided at the top of every notebook. If the import statements do not work, then you will have to run a !pip install for each individual libraries. These are not included at the top of each notebook.
